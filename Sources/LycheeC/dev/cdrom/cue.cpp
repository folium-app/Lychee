...
void cue_parse_index(cue_t* cue) {
    cue_track_t* track = (cue_track_t*)list_back(cue->tracks)->data;
...
cue_track_t* cue_parse_track(cue_t* cue) {
    ...
    cue_track_t* track = (cue_track_t*)malloc(sizeof(cue_track_t));
    ...
    track->index[0] = -1;
    track->index[1] = -1;
    track->file = (cue_file_t*)list_back(cue->files)->data;
...
cue_file_t* cue_parse_file(cue_t* cue, const char* p, const char* s) {
    ...
    cue_file_t* file = (cue_file_t*)malloc(sizeof(cue_file_t));
...
cue_t* cue_create(void) {
    return (cue_t*)malloc(sizeof(cue_t));
}
    ...
    while (!feof(cue->file)) {
        ...
        switch (kw) {
            ...
            case CUE_TRACK: {
                ...
                cue_file_t* file = (cue_file_t*)list_back(cue->files)->data;
...
uint32_t init_tracks(cue_file_t* file, uint32_t* lba) {
    ...
    if (file->tracks->size == 1) {
        cue_track_t* data = (cue_track_t*)node->data;
    ...
    while (node) {
        ...
        cue_track_t* next = (cue_track_t*)node->next->data;
...
int cue_load(cue_t* cue, int mode) {
    ...
    while (node) {
        cue_file_t* data = (cue_file_t*)node->data;
...
void cue_destroy(cue_t* cue) {
    ...
    while (node) {
        cue_file_t* file = (cue_file_t*)node->data;
...
cue_track_t* get_sector_track(cue_t* cue, uint32_t lba) {
    ...
    while (node) {
        cue_track_t* track = (cue_track_t*)node->data;
...
int cue_read(cue_t* cue, uint32_t lba, void* buf) {
    ...
    if (file->buf_mode == LD_BUFFERED) {
        ...
        fseek((FILE*)file->buf, (lba - file->start) * 2352, SEEK_SET);

        // Should always succeed, ignore result for speed
        (void)fread(buf, 1, 2352, (FILE*)file->buf);
    }
...
int cue_get_track_lba(cue_t* cue, int track) {
    ...
    cue_track_t* data = (cue_track_t*)list_at(cue->tracks, track - 1)->data;
