...
void mdec_decode_macroblock(psx_mdec_t* mdec) {
    if (mdec->output_depth < 2) {
        ...
        mdec->output = (uint8_t*)malloc(size);
        ...
        while (bytes_processed < mdec->input_size) {
            if (!mdec->output) {
                mdec->output = (uint8_t*)malloc(block_count * size);
            } else {
                mdec->output = (uint8_t*)realloc(mdec->output, block_count * size);
            }
    ...
    switch (offset) {
        case 0: {
            ...
            if (mdec->words_remaining) {
                ...
                mdec->input_full = 0;
                mdec->input_index = 0;
                mdec->input = (uint32_t*)malloc(mdec->input_size);
            }
        } break;
